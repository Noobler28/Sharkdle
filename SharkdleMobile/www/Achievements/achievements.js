...
let activeAchievementCategory = "worldmap";
let activeAchievementFilter = "all";

const achievementCategoryMeta = [
    { key: "worldmap", label: "World Map", icon: "fa-map-location-dot" },
    { key: "special", label: "Special", icon: "fa-star" },
    { key: "duels", label: "Duels", icon: "fa-handshake" },
    { key: "wins", label: "Wins", icon: "fa-trophy" },
    { key: "games", label: "Games", icon: "fa-gamepad" },
    { key: "streaks", label: "Streaks", icon: "fa-fire" },
    { key: "efficiency", label: "Efficiency", icon: "fa-bullseye" },
...
function renderAchievements(profileData, unlockedAchievements) {
    ...
    const categories = achievementCategoryMeta.filter(category => Array.isArray(achievementDefinitions[category.key]) && achievementDefinitions[category.key].length > 0);
    if (!categories.some(category => category.key === activeAchievementCategory)) {
        activeAchievementCategory = categories[0]?.key || "worldmap";
    }

    renderAchievementCategoryNav(categories, unlockedAchievements);
    bindAchievementFilterButtons();
    renderAchievementActiveSummary(activeAchievementCategory, unlockedAchievements);
    container.appendChild(createCategorySection(getAchievementCategoryLabel(activeAchievementCategory), activeAchievementCategory, profileData, unlockedAchievements));
    return;
...
function getAchievementCategoryLabel(categoryKey) {
    return achievementCategoryMeta.find(category => category.key === categoryKey)?.label || categoryKey;
}

function getAchievementCardState(achievement, unlockedAchievements) {
    const claimedAchievements = getStoredAchievementIds("claimedAchievements");
    if (claimedAchievements.includes(achievement.id)) return "claimed";
    if (unlockedAchievements.includes(achievement.id)) return "ready";
    return "locked";
}

function getFilteredAchievements(categoryKey, unlockedAchievements) {
    const achievements = getCategoryAchievements(categoryKey);
    if (activeAchievementFilter === "ready") {
        return achievements.filter(achievement => getAchievementCardState(achievement, unlockedAchievements) === "ready");
    }
    if (activeAchievementFilter === "locked") {
        return achievements.filter(achievement => getAchievementCardState(achievement, unlockedAchievements) === "locked");
    }
    return achievements;
}

function renderAchievementCategoryNav(categories, unlockedAchievements) {
    const nav = document.getElementById("achievement-category-nav");
    if (!nav) return;

    nav.innerHTML = categories.map(category => {
        const achievements = getCategoryAchievements(category.key);
        const unlockedCount = achievements.filter(achievement => unlockedAchievements.includes(achievement.id)).length;
        const readyCount = achievements.filter(achievement => getAchievementCardState(achievement, unlockedAchievements) === "ready").length;
        const progress = achievements.length ? Math.round((unlockedCount / achievements.length) * 100) : 0;
        return `
            <button type="button" class="achievement-category-tab ${category.key === activeAchievementCategory ? "active" : ""}" data-achievement-category="${category.key}">
                <span class="achievement-category-tab-icon"><i class="fas ${category.icon}"></i></span>
                <span class="achievement-category-tab-copy">
                    <strong>${category.label}</strong>
                    <small>${unlockedCount}/${achievements.length} unlocked${readyCount ? ` - ${readyCount} ready` : ""}</small>
                </span>
                <span class="achievement-category-tab-progress"><span style="width:${progress}%"></span></span>
            </button>
        `;
    }).join("");

    nav.querySelectorAll("[data-achievement-category]").forEach(button => {
        button.addEventListener("click", async () => {
            activeAchievementCategory = button.dataset.achievementCategory;
            await loadAndDisplayAchievements();
        });
    });
}

function bindAchievementFilterButtons() {
    document.querySelectorAll("[data-achievement-filter]").forEach(button => {
        button.classList.toggle("active", button.dataset.achievementFilter === activeAchievementFilter);
...
function renderAchievementActiveSummary(categoryKey, unlockedAchievements) {
    const summary = document.getElementById("achievement-active-summary");
    if (!summary) return;

    const allCategoryAchievements = getCategoryAchievements(categoryKey);
    const unlockedCount = allCategoryAchievements.filter(achievement => unlockedAchievements.includes(achievement.id)).length;
    const claimedCount = allCategoryAchievements.filter(achievement => getAchievementCardState(achievement, unlockedAchievements) === "claimed").length;
    const readyCount = allCategoryAchievements.filter(achievement => getAchievementCardState(achievement, unlockedAchievements) === "ready").length;
    const meta = achievementCategoryMeta.find(category => category.key === categoryKey);
    const progress = allCategoryAchievements.length ? Math.round((unlockedCount / allCategoryAchievements.length) * 100) : 0;

    summary.innerHTML = `
        <div class="achievement-active-header">
            <div class="achievement-active-title">
                <span><i class="fas ${meta?.icon || "fa-award"}"></i></span>
                <div>
                    <h2>${meta?.label || categoryKey}</h2>
                    <p>${progress}% complete in this category</p>
                </div>
            </div>
            <div class="achievement-active-metrics">
                <span>${unlockedCount}/${allCategoryAchievements.length} unlocked</span>
                <span>${claimedCount} claimed</span>
                <span>${readyCount} ready</span>
            </div>
        </div>
        <div class="achievement-active-progress" aria-hidden="true"><span style="width:${progress}%"></span></div>
    `;
}
...
function createCategorySection(categoryName, categoryKey, profileData, unlockedAchievements) {
    ...
    const allCategoryAchievements = getCategoryAchievements(categoryKey);
    const achievements = getFilteredAchievements(categoryKey, unlockedAchievements);
    const unlockedCount = allCategoryAchievements.filter(achievement => unlockedAchievements.includes(achievement.id)).length;
    ...
    meta.textContent = `${unlockedCount}/${allCategoryAchievements.length} unlocked`;
    ...
    if (achievements.length) {
        ...
        empty.className = "achievement-empty-state";
        empty.innerHTML = `
            <i class="fas fa-magnifying-glass"></i>
            <h3>No achievements here</h3>
            <p>Try another filter or category.</p>
        `;
